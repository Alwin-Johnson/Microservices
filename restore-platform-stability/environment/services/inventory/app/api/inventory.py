...
async def reserve_inventory(
    ...
    try:
        return await service.reserve_stock(
            order_id=request.order_id, sku=request.sku, quantity=request.quantity
        )
...
async def release_inventory(
    reservation_id: UUID, service: InventoryService = Depends(get_inventory_service)
):
    try:
...
async def get_inventory(
    sku: str, service: InventoryService = Depends(get_inventory_service)
):
    try:
