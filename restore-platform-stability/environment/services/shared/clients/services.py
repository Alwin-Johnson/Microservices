...
class OrdersClient(BaseHttpClient):
    ...
    async def create_order(self, customer_id: str, total_amount: float) -> Dict[str, Any]:
        return await self.post("/orders", json={"customer_id": customer_id, "total_amount": total_amount})
...
class PaymentsClient(BaseHttpClient):
    ...
    async def process_payment(self, order_id: str, amount: float) -> Dict[str, Any]:
        return await self.post("/payments", json={"order_id": order_id, "amount": amount})
...
class InventoryClient(BaseHttpClient):
    ...
    async def reserve_inventory(self, order_id: str, sku: str, quantity: int) -> Dict[str, Any]:
        return await self.post("/inventory/reserve", json={
            "order_id": order_id,
            "sku": sku,
            "quantity": quantity
        })
